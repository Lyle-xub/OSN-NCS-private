...
class ComplexResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out = torch.stack(
            [torch.real(x_out).double(),
             torch.imag(x_out).double()], dim=0)
        return x_out
...
if __name__ == "__main__":
    # Test ComplexUNet
    import time
    # Generate a random complex input
    real_part = torch.randn((700, 700))
    imag_part = torch.randn((700, 700))
    x = torch.complex(real_part, imag_part)
    batch_size = 8
    x = torch.stack([x] * batch_size, dim=0).cuda()
    net = ComplexUNet((700, 700), 7, True, [3, 3, 3], [8, 16, 32]).cuda()
